...
resource "azurerm_virtual_machine" "master" {
  ...
  storage_data_disk {
    name            = "${azurerm_managed_disk.master_docker.name}"
    managed_disk_id = "${azurerm_managed_disk.master_docker.id}"
    create_option   = "Attach"
    lun             = 0
    disk_size_gb    = "${azurerm_managed_disk.master_docker.disk_size_gb}"
  }
